...
pub fn load_config_file<P: AsRef<Path>>(images_dir: P, action: &str) -> ClientConfig {
    ...
    if is_dump_action(action) {
        ...
        // We first look for container ID.
        // If the PID is not a container, we consider it as a regular process.
        let mut id = find_container_id_from_pid(pid).unwrap();
        if id.is_empty() {
            let process_name = format!("/proc/{pid}/comm");
            if let Ok(name) = fs::read_to_string(process_name) {
                id = name.trim().to_string();
            } else {
                // Default to using the PID as the ID
                id = pid.to_string();
            }
        }
        ...
        // Write the local config for each container during the first dump action
