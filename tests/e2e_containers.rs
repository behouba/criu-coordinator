...
fn e2e_container_tcp_checkpoint_restore() {
    ...
    // --- STOP AND REMOVE CONTAINERS FOR RESTORE ---
    println!("\n--- Stopping and removing containers for restore ---");
    run_command(
    ...
    run_command(
        Command::new("podman").args(["container", "rm", CLIENT_CONTAINER_NAME]),
        "Failed to remove client container",
    );
    run_command(
        Command::new("podman").args(["container", "rm", SERVER_CONTAINER_NAME]),
        "Failed to remove server container",
    );
    ...
    assert!(
        !server_logs_after.contains("SERVER_ERROR"),
