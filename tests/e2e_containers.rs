...
//  - Use podman to create two containers one running a server and the other running the client
//  - Ensure the containers are communicating over TCP
//  - Checkpoint the client and then the server while preserving the connection state with tcp-established
//  - Remove the containers
//  - Try to recreate and restore the containers
...
/// Sets up the entire test environment: binaries, images, network, and containers.
fn setup_container_test() {
    println!("\n--- Setting up Podman container test environment ---");
...
fn e2e_container_tcp_checkpoint_restore() {
    ...
    // Attempts to restore containers again 5 times if connection errors are found
    for i in 0..5 {
        // remove the containers and retry to restore again if there are errors
        cleanup_containers();
        ...
        println!("\n--- Starting RESTORE phase at attempt {} ---", i + 1);
        ...
        if client_logs_after.contains("CLIENT_ERROR")  || server_logs_after.contains("SERVER_ERROR") {
            println!("Connection errors still present in logs after retrying restore {}.", i + 1  );
            if i == 4 {
                panic!("Test failed after 5 retries to restore containers with connection errors.");
            }
        } else {
            if i > 0 {
                panic!("[WARNING] Test passed after {} retries", i + 1);
            } else {
                println!("[INFO] Test passed without retries");
                break;
            }
        }
    }
